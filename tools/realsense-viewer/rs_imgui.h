...
// Window/Popup tracking macros
#ifdef RS_DUMP_UI
inline bool UI_Begin(const char* name, bool* p_open = nullptr, ImGuiWindowFlags flags = 0) {
    bool result = ImGui::Begin(name, p_open, flags);
    if (result) {
        ImGuiWindow* window = ImGui::GetCurrentWindow();
        ui_dump_on_begin_window(name, window->ID, (flags & ImGuiWindowFlags_AlwaysVerticalScrollbar) || (flags & ImGuiWindowFlags_AlwaysHorizontalScrollbar));
    }
    return result;
}
inline void UI_End() {
...
inline bool UI_BeginPopup(const char* str_id, ImGuiWindowFlags flags = 0) {
...
inline void UI_EndPopup() {
    ui_dump_on_end_popup();
    ImGui::EndPopup();
}
inline bool UI_BeginChild(const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags flags = 0) {
    bool result = ImGui::BeginChild(str_id, size, border, flags);
    if (result) {
        ImGuiWindow* window = ImGui::GetCurrentWindow();
        ui_dump_on_begin_child(str_id, window->ID, (flags & ImGuiWindowFlags_AlwaysVerticalScrollbar) || (flags & ImGuiWindowFlags_AlwaysHorizontalScrollbar));
    }
    return result;
}
inline void UI_EndChild() {
    ui_dump_on_end_child();
    ImGui::EndChild();
}
#else
#define UI_Begin ImGui::Begin
#define UI_End ImGui::End
#define UI_BeginPopup ImGui::BeginPopup
#define UI_EndPopup ImGui::EndPopup
#define UI_BeginChild ImGui::BeginChild
#define UI_EndChild ImGui::EndChild
#endif
