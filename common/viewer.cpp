...
namespace rs2
{
    ...
    void viewer_model::set_export_popup(ImFont* large_font, ImFont* font, rect stream_rect, std::string& error_message, config_file& temp_cfg)
    {
        ...
        if (UI_BeginPopupModal("Export", nullptr, flags))
        {
            ...
            if (tab == export_type::ply)
            {
                ...
                ImGui::PopStyleColor();
                if (UI_RadioButton("Textual", encoding == configurations::ply::textual))
                {
                    encoding = configurations::ply::textual;
                    temp_cfg.set(configurations::ply::encoding, encoding);
                }
                if (UI_RadioButton("Binary", encoding == configurations::ply::binary))
                {
        ...
        )
    {
        ...
        if (dropdown)
        {
            clicked = clicked || UI_Button(textual_icons::chevron_down, { font_size, 55 } );
            hovered = hovered || ImGui::IsItemHovered();
        }
    ...
    void viewer_model::show_3dviewer_header(ux_window& win, rs2::rect stream_rect, bool& paused, std::string& error_message)
    {
        ...
        if (UI_BeginPopup(source_selection_popup))
        {
        ...
        ImGui::SetNextWindowPos({ cursor.x + left + 5, cursor.y + 60 });
        if (UI_BeginPopup(tex_selection_popup))
        {
        ...
        ImGui::SetNextWindowPos({ cursor.x + left + 5, cursor.y + 60 });
        if (UI_BeginPopup(shader_selection_popup))
        {
        ...
        if (last_labeled_points)
        {
            ...
            if (UI_BeginPopup(lpc_popup))
            {
                ...
                if (UI_MenuItem("Medium", nullptr, &selected))
                {
                ...
                if (UI_MenuItem("Large", nullptr, &selected))
                {
    ...
    void rs2::viewer_model::show_popup(const ux_window& window, const popup& p)
    {
        ...
        if (UI_BeginPopupModal(p.header.c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize))
        {
    ...
    rs2::frame viewer_model::handle_ready_frames(const rect& viewer_rect, ux_window& window, int devices, std::string& error_message)
    {
        ...
        UI_Begin("Viewport", nullptr,flags);
    ...
    void viewer_model::show_top_bar(ux_window& window, const rect& viewer_rect, const device_models_list& devices)
    {
        ...
        UI_Begin("Toolbar Panel", nullptr, flags);
        ...
        if (UI_Button(textual_icons::cog, { panel_y,panel_y }))
        {
        ...
        if (window.is_fullscreen())
        {
            ...
            if (UI_Button(textual_icons::power_off, { panel_y,panel_y }))
            {
                exit(0);
            }
            if (ImGui::IsItemHovered())
...

        {
            ...
            if (UI_BeginPopupModal(menu_items[2], nullptr, flags))
            {
                ...
                if (UI_BeginChild("ScrollableRegion", child_size, true, ImGuiWindowFlags_AlwaysVerticalScrollbar))
                {
                if (tab == 0)
                {
                    int recording_setting = temp_cfg.get(configurations::record::file_save_mode);
                    ImGui::Text("When starting a new recording:");
                    if (UI_RadioButton("Select filename automatically", recording_setting == 0))
                    {
                        recording_setting = 0;
                        temp_cfg.set(configurations::record::file_save_mode, recording_setting);
                    }
                    if (UI_RadioButton("Ask me every time", recording_setting == 1))
                    {
                        recording_setting = 1;
                        temp_cfg.set(configurations::record::file_save_mode, recording_setting);
                    }
                    ...
                    if (UI_InputText("##default_record_path", path, 255))
                    {
                        path_str = path;
                    ...
                    if (UI_RadioButton("Always Compress (might cause frame drops)", recording_compression == 0))
                    {
                        recording_compression = 0;
                        temp_cfg.set(configurations::record::compression_mode, recording_compression);
                    }
                    if (UI_RadioButton("Never Compress (larger .bag file size)", recording_compression == 1))
                    {
                        recording_compression = 1;
                        temp_cfg.set(configurations::record::compression_mode, recording_compression);
                    }
                    if (UI_RadioButton("Use device defaults", recording_compression == 2))
                    {
                ...
                if (tab == 1)
                {
                    ...
                    if (UI_SliderInt("##font_samples", &font_samples, 1, 8))
                    {
                    ...
                    if( UI_SliderInt( "##font_size", &font_size, 16, 20 ) )
                    {
                        reload_required = true;
                    ...
                    if (msaa)
                    {
                        ...
                        if (UI_SliderInt("##samples", &samples, 2, 16))
                        {
                            reload_required = true;
                ...
                if( tab == 2 )
                {
                    ...
                    if (log_to_file)
                    {
                        ...
                        if (UI_InputText("##default_log_path", logpath, 255))
                        {
                            path_str = logpath;
...

                    {
                        ...
                        if (UI_InputText("##commands_xml_path", logpath, 255))
                        {
                            path_str = logpath;
...

                    {
                        ...
                        if (UI_InputText("##fw_log_xml_path", logpath, 255))
                        {
                            path_str = logpath;
                    ...
                    if( enable_dds )
                    {
                        ...
                        ImGui::SameLine();
                        if( UI_InputInt( "##Domain ID", &domain_id ) )
                        {
                            if( domain_id < 0 )
                                domain_id = 0;
                ...
                if (tab == 3)
                {
                    ...
                    if (UI_RadioButton("Official Server", official_url))
                    {
                        official_url = true;
                    ...
                    ImGui::SameLine();
                    if (UI_RadioButton("Custom Server", !official_url))
                    {
                        official_url = false;
                    ...
                    if (!official_url)
                    {
                        if (UI_InputText("##custom_server_url", custom_url, 255))
                        {
                            url_str = custom_url;
                            temp_cfg.set(configurations::update::sw_updates_url, url_str);
                        }
...

        {
            ...
            if (UI_BeginPopupModal(menu_items[3], nullptr, flags))
            {
