...
#else
    if (UI_DragFloat(std::string( rsutils::string::from() << "##" << name).c_str(), &x, 0.001f))
#endif
    {
        changed = true;
    }
...
void calibration_model::update(ux_window& window, std::string& error_message)
{
    ...
    if (UI_BeginPopupModal(window_name, nullptr, flags))
    {
        ...
        UI_BeginChild("##CalibData",ImVec2(w - 15, h - 110), true);
        ...
        ImGui::PushItemWidth(120);
        UI_Combo("##RectifiedResolutions", &selected_resolution, resolution_names_char.data(), int(resolution_names_char.size()));
#ifdef RS_DUMP_UI
